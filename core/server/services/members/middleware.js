...
const getIdentityToken = async function (req, res) {
    try {
        const token =
            await membersService.ssr.getIdentityTokenForMemberFromSession(
                req,
                res
            );
        res.writeHead(200);
...
const getMemberData = async function (req, res) {
    try {
        ...
        if (userId) {
            Object.assign(req, { member: userId });
            res.locals.member = req.member;
            res.json(formattedMemberResponse(userId));
        } else {
            res.json(null);
        }
    } catch (err) {
...
const updateMemberData = async function (req, res) {
    try {
        ...
        const member = await membersService.ssr.getMemberDataFromSession(
            req,
            res
        );
        if (member) {
...
const getMemberSiteData = async function (req, res) {
    ...
    const firstpromoterId = settingsCache.get("firstpromoter")
        ? settingsCache.get("firstpromoter_id")
        : "";
    const blogDomain = domain && domain[1];
    let supportAddress =
        settingsCache.get("members_support_address") || "noreply";
    if (!supportAddress.includes("@")) {
        supportAddress = `${supportAddress}@${blogDomain}`;
    }
    const { products = [], prices = [] } =
        (await getPortalProductPrices()) || {};
    ...
    const response = {
        ...
        free_price_description: settingsCache.get(
        ...
        portal_button_signup_text: settingsCache.get(
...
const createSessionFromMagicLink = async function (req, res, next) {
    ...
    try {
        const member = await membersService.ssr.exchangeTokenForSession(
            req,
            res
        );
        ...
        if (action === "signup") {
            ...
            if (
                ...
                customRedirect =
                    settingsCache.get("members_paid_signup_redirect") || "";
            } else {
                customRedirect =
                    settingsCache.get("members_free_signup_redirect") || "";
            }
...
module.exports = {
    ...
    stripeWebhooks: (req, res, next) =>
        membersService.api.middleware.handleStripeWebhook(req, res, next),
};
